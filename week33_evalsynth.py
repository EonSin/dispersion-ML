...
for i in tqdm(range(1, 0, -1)):#tqdm([8,4,1]):
  ...
  wanted_model = 'synth_models/1/synth-stacked-64559_0.002208594262599945_.h5'
  model = torch.load(wanted_model)
  #train_loss, valid_loss = model.train(train_loader, val_loader, patience, n_epochs)

  y_pred = model.eval(train_loader).transpose((0,2,3,1))#val_loader).transpose((0,2,3,1))
  ...
  y_mids = y_img[:, :, :, :]
  pred_savepath = 'pics/'
  Image.fromarray(y_mids[0]).save(pred_savepath+'pred.png')
  
  N = len(y_pred)
  y_pred = np.argmax(y_pred, axis=-1)
  y_pred = np.reshape(y_pred, (N, 64*64))
  
  y_actual = Y_synth[train_idx]#validation_idx]
  y_img = np.round(squash(0,1, 0,255, y_actual)).astype('uint8') # predictions
  y_img[np.where((y_img==[255,0,0]).all(axis=3))] = [255,255,255] # red -> white
  # %% filtering!
  # y_img[(np.max(y_img, axis=-1)<255)] = 255 # "if you aren't 100% sure, then you are noise"
  # y_img = y_img.astype('uint8')
  y_mids = y_img[:, :, :, :]
  pred_savepath = 'pics/'
  Image.fromarray(y_mids[0]).save(pred_savepath+'actual.png')
